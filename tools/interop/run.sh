...
# Only reap a peer already known to have exited; a live peer without an
# endpoint is a startup failure, never an unbounded wait.
exit_without_endpoint() {
    if kill -0 "$peer_pid" 2>/dev/null; then exit 1; fi
    if wait "$peer_pid"; then exit 1; else exit $?; fi
}
endpoint=""
for attempt in {1..100}; do
    endpoint="$(sed -n '1p' "$peer_log")"
    if [[ "$endpoint" == ws://127.0.0.1:* ]]; then break; fi
    if ! kill -0 "$peer_pid" 2>/dev/null; then exit_without_endpoint; fi
    sleep 0.1
done
if [[ "$endpoint" != ws://127.0.0.1:* ]]; then exit_without_endpoint; fi
