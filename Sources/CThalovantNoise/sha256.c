...
static void sha256_block(thalovant_sha256_ctx *ctx, const uint8_t block[64])
{
    uint32_t w[64];
    for (int i = 0; i < 16; i++) {
...
void thalovant_sha256_final(thalovant_sha256_ctx *ctx, uint8_t out[32])
{
    ...
    uint8_t pad = 0x80;
    thalovant_sha256_update(ctx, &pad, 1);
    uint8_t zero = 0x00;
    while (ctx->buffered != 56) {
        thalovant_sha256_update(ctx, &zero, 1);
    }
    uint8_t length_be[8];
    for (int i = 0; i < 8; i++) {
        length_be[i] = (uint8_t)(bit_length >> (56 - i * 8));
    }
    /* Bypass the length accumulator for the padding tail. */
    memcpy(ctx->buffer + 56, length_be, 8);
    sha256_block(ctx, ctx->buffer);
    for (int i = 0; i < 8; i++) {
...
{
    thalovant_sha256_ctx ctx;
    thalovant_sha256_init(&ctx);
    thalovant_sha256_update(&ctx, data, len);
    thalovant_sha256_final(&ctx, out);
}
